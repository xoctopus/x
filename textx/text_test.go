...
func TestUnmarshalText(t *testing.T) {
	cases := map[string]struct {
		...
		"Int":                 {[]byte("1"), new(int), new(1)},
		"Int2":                {[]byte("1"), new(*int), new(new(1))},
		"Uint":                {[]byte("1"), new(uint), new(uint(1))},
		"Float":               {[]byte("1.1"), new(float64), new(1.1)},
		...
		"Bytes2":              {[]byte("any"), new(testdata.Bytes), new(testdata.Bytes("any"))},
		"Arshaler1":           {[]byte("1"), new(testdata.Duration), ptrx.Ptr(testdata.Duration(1))},
		"Arshaler2":           {[]byte{1, 0, 0, 0, 0, 0, 0, 0}, new(testdata.Integers), new(testdata.Integers{1})},
	}
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
