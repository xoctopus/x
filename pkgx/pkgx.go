...
func New(pkg *packages.Package) *Pkg {
	imports := &Set{}
	imports.Append(pkg)
	return &Pkg{
		Package: pkg,
...
func LoadFrom(pattern string) (*Pkg, error) {
	lst, err := packages.Load(
		&packages.Config{
			Mode:    packages.LoadMode(0b111111111111),
			Tests:   true,
...
func (p *Pkg) Imports() []*packages.Package {
	return p.imports
}
...
func (p *Pkg) PkgByPath(path string) *packages.Package {
...
func (p *Pkg) PkgByPos(n Pos) *packages.Package {
	if n == nil || reflectx.IsZero(n) {
		return nil
	}
	for _, pkg := range p.imports {
...
func (p *Pkg) IdentOf(obj types.Object) *ast.Ident {
	if obj == nil || reflectx.IsZero(obj) {
		return nil
	}
...
func (p *Pkg) FuncResults(fn *types.Func) (Results, int) {
	if fn == nil {
		return nil, 0
	}
...
func (s Set) Append(pkg *packages.Package) {
	s[pkg.ID] = pkg
...
func (s Set) List() (ret []*packages.Package) {
	for id := range s {
		ret = append(ret, s[id])
	}
	return ret
